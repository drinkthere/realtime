...
import capital.daphne.datasource.ibkr.adapter.AdvisorUtil;
import capital.daphne.datasource.ibkr.adapter.ConcurrentHashSet;
...
public class IbkrController implements EWrapper {
    ...
    public IbkrController(IConnectionHandler handler) {
        this(handler, null, null);
    }

    public IbkrController(IConnectionHandler handler, ILogger inLogger, ILogger outLogger) {
    ...
    public void tickGeneric(int reqId, int tickType, double value) {
        ITopMktDataHandler handler = m_topMktDataMap.get(reqId);
        if (handler != null) {
            handler.tickPrice(reqId, TickType.get(tickType), value, new TickAttrib());
        }
        recEOM();
    }
    ...
    public int reqRealTimeBars(Contract contract, WhatToShow whatToShow, boolean rthOnly, IRealTimeBarHandler handler) {
        if (!checkConnection())
            return 0;
    ...
    public interface ITopMktDataHandler {
        void tickPrice(int reqId, TickType tickType, double price, TickAttrib attribs);
    ...
    public interface IRealTimeBarHandler {
        void realtimeBar(int reqId, com.ib.controller.Bar bar); // time is in seconds since epoch
    }
    ...
    public static class TopMktDataAdapter implements ITopMktDataHandler {
        @Override
        public void tickPrice(int reqId, TickType tickType, double price, TickAttrib attribs) {
        }
